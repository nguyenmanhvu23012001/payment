...
class TransactionService {
  ...
  Response response;
    response = await dio.post(baseURL+'/slink/v1/transactions',data:data );

   if(response.statusCode == 200){
     var reponseData = jsonDecode(response.data);
     var transactionData = reponseData['data'];
     TransactionModel transaction = TransactionModel(
       ...
       transactionmoney: transactionData['transaction_money']
     );
  ...
  Future updatePost(String reponseString) async{
    ...
    final uri = Uri.http('192.168.55.121:8080', '/slink/v1/transactions',);
